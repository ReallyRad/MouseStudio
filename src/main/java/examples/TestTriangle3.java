...
/**
 * Created by mar on 18.12.14.
 */
public class TestTriangle3 extends PApplet {
    ...
    public void draw() {
        ...
        for( int i = 0; i < displayAtOnce; i++ ) {
            MousePath morphedA = mm.get( selectedPath ).getPositionsScaledAndRotated( edgeA, edgeB );
            MousePath morphedB = mm.get( selectedPath + 1 ).getPositionsScaledAndRotated( edgeB, edgeC );
            MousePath morphedC = mm.get( selectedPath + 2 ).getPositionsScaledAndRotated( edgeC, edgeA );


            mv.drawRawPath( morphedA );
            mv.drawRawPath( morphedB );
            mv.drawRawPath( morphedC );
    ...
    public static void main( String[] args ) {
        PApplet.main( new String[]{ "examples.TestTriangle3" } );
    }
}
